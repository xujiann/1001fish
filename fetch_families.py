...
"""为 fish.js 中 family 为空的条目（cat=more 的 762 条）从 Wikidata 补科名。
   分块查询（学名 VALUES → 科的拉丁名 P225 + 中文标签），写 _families.json {sci:{la,zh}}。"""
...
def load_more_scis():
    ...
    d = json.loads(arr)
    return [f["sci"] for f in d if f.get("cat") == "more" and not f.get("family")]
...
def query(scis, tries=6):
    ...
    q = ("SELECT ?sci ?famLa ?famZh WHERE { VALUES ?sci { %s } "
         ...
         "?fam wdt:P105 wd:Q35409 ; wdt:P225 ?famLa . "
...
def main():
    scis = load_more_scis()
    print("need families for", len(scis), "species")
    out = {}
    if os.path.exists(os.path.join(ROOT, "_families.json")):
        out = json.load(open(os.path.join(ROOT, "_families.json"), encoding="utf-8"))
    ...
    for i in range(0, len(todo), CHUNK):
        ...
        for r in rows:
            ...
            got += 1
        json.dump(out, open(os.path.join(ROOT, "_families.json"), "w", encoding="utf-8"),
                  ensure_ascii=False, indent=0)
        print("  chunk %d-%d: +%d (total %d)" % (i, i+len(chunk), got, len(out)))
        time.sleep(2)
    print("=== done: %d/%d have family ===" % (len([s for s in scis if s in out]), len(scis)))
